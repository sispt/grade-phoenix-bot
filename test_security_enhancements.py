...
def test_rate_limiting():
    ...
    try:
        ...
        for i in range(5):
            ...
            if i < 4:  # First 4 attempts should still allow the next one
                assert rate_limiter.is_allowed(
                    user_id
                ), f"User should be allowed after {i+1} attempts"

        # After 5 attempts, user should be blocked
        assert not rate_limiter.is_allowed(
            user_id
        ), "User should be blocked after 5 attempts"
        ...
        )  # This counts as 3 more attempts
        assert not rate_limiter.is_allowed(
            user_id
        ), "User should be blocked after 2 failed attempts"
...
def test_session_management():
    ...
    try:
        ...
        assert (
            session["user_data"]["username"] == "test_user"
...
def test_security_manager():
    ...
    try:
        ...
        user_id = 12345
        assert security_manager.check_login_attempt(
            user_id
        ), "Login attempt should be allowed initially"
...
def test_welcome_messages():
    ...
    try:
        ...
        assert (
            "بوت متابعة الدرجات الجامعية" in simple_welcome
        ), "Simple welcome should explain what the bot does"
        assert (
            "🚀 تسجيل الدخول" in simple_welcome
        ), "Simple welcome should mention login button"

        # Test security welcome message
        security_welcome = get_security_welcome_message()
        assert (
            "مرحباً بعودتك" in security_welcome
        ), "Security welcome should contain return greeting"
        assert (
            "حالة الأمان" in security_welcome
        ), "Security welcome should mention security status"
        assert (
            "/security_info" in security_welcome
        ), "Security welcome should mention security commands"

        # Test regular welcome message
        regular_welcome = get_welcome_message("Test User")
        assert (
            "أهلاً Test User" in regular_welcome
        ), "Regular welcome should include user name"
        assert (
            "ما هو هذا البوت" in regular_welcome
        ), "Regular welcome should explain what the bot does"
        assert (
            "ماذا يمكنك أن تفعل" in regular_welcome
        ), "Regular welcome should list features"
