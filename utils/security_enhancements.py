...
class AuditLogger:
    ...
    def log_security_event(
        ...
        if risk_level in ["HIGH", "CRITICAL"]:
            logger.warning(
                f"SECURITY ALERT: {event_type} - User {user_id} - {risk_level}"
            )
...
class SessionManager:
    ...
    def create_session(
        self, user_id: int, token: str, user_data: Dict[str, Any] | None = None
    ):
        ...
        # Check session limit
        if (
            len([s for s in self.sessions.values() if s.get("user_id") == user_id])
...
class SecurityManager:
    ...
    def check_login_attempt(
        self, user_id: int, ip_address: Optional[str] = None
    ) -> bool:
    ...
    def record_login_attempt(
        ...
        self.audit_logger.log_security_event(
            ...
            details={
                "username": username,
                "attempts": self.rate_limiter.get_attempts_count(user_id),
            },
            success=success,
            risk_level=risk_level,
            ip_address=ip_address,
    ...
    def get_security_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_logins": len(
                [e for e in recent_events if e.event_type == "LOGIN_FAILED"]
            ),
            "blocked_attempts": len(
                [e for e in recent_events if e.event_type == "LOGIN_BLOCKED"]
            ),
            "active_sessions": len(
                [
                    s
...
def is_valid_length(
    value: str, min_len: int | None = None, max_len: int | None = None
) -> bool:
