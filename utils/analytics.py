...
class GradeAnalytics:
    ...
    async def _try_adviceslip_api(self) -> Optional[Dict[str, str]]:
        ...
        try:
            async with asyncio.timeout(5):
                response = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda: requests.get("https://api.adviceslip.com/advice", timeout=5)
                )
                if response.status_code == 200:
                    data = response.json()
    ...
    async def format_old_grades_with_analysis(self, telegram_id: int, old_grades: List[Dict[str, Any]]) -> str:
...
"""
            ...
            message += "🔍 *استخدم هذا التحليل لفهم أدائك السابق وتحسين مستقبلك الأكاديمي*"
