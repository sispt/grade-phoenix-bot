"""
📝 Message Templates (Enhanced UX)
Improved user experience with clearer explanations and helpful content
"""

from config import CONFIG
from storage.models import Base, DatabaseManager


def get_welcome_message(fullname: str | None = None) -> str:
    """Returns a comprehensive, user-friendly welcome message explaining what the bot does."""
    name_line = f"أهلاً وسهلاً {fullname}! 👋\n\n" if fullname else "مرحباً بك! 👋\n\n"

    return (
        name_line
        + "🎓 **ما هو هذا البوت؟**\n"
        + "بوت ذكي لمتابعة درجاتك الجامعية بسهولة وأمان!\n\n"
        + "✨ **الميزات الرئيسية:**\n"
        + "• 📊 عرض درجاتك الحالية والسابقة\n"
        + "• 🔔 إشعارات فورية عند تحديث الدرجات\n"
        + "• 📱 واجهة سهلة الاستخدام\n"
        + "• 🔐 حماية كاملة لبياناتك\n\n"
        + "🚀 **للبدء:**\n"
        + "اضغط على '🚀 تسجيل الدخول للجامعة' وأدخل بياناتك\n\n"
        + "🔐 **الأمان:**\n"
        + "• تشفير كامل للبيانات\n"
        + "• معايير أمان عالية (OWASP + NIST)\n"
        + "• لا نخزن كلمات المرور نهائياً\n"
        + "• لا يتم فحص أو جلب درجاتك أو الاطﻻع عليها من قبل المطور إلا عندما تطلب ذلك بنفسك.\n\n"
        + f"👨‍💻 المطور: {CONFIG['ADMIN_USERNAME']}\n"
        + "\n🌐 [رابط الكود البرمجي (GitHub) - للشفافية الكاملة ومعرفة كيف يعمل البوت](https://github.com/sispt/grade-phoenix-bot)\n"
        + "\nإذا أردت تحميل بياناتك يمكنك ذلك من الإعدادات أو التواصل مع المطور (ﻻ يشمل كلمة المرور لأننا ﻻ نحتفظ بها، فقط بيانات الدرجات)."
    )


def get_simple_welcome_message() -> str:
    """Returns a clear and concise welcome message for new users."""
    return (
        "مرحباً! 👋\n\n"
        + "🎓 **بوت متابعة الدرجات الجامعية**\n\n"
        + "**ماذا يفعل البوت؟**\n"
        + "• يعرض درجاتك الجامعية بسهولة\n"
        + "• يخبرك فوراً عند تحديث الدرجات\n"
        + "• واجهة بسيطة وسهلة الاستخدام\n"
        + "• حماية كاملة لبياناتك\n\n"
        + "**كيف تبدأ؟**\n"
        + "اضغط '🚀 تسجيل الدخول للجامعة' وأدخل بياناتك الجامعية\n\n"
        + "🔐 **الأمان:**\n"
        + "بياناتك مشفرة ومحمية بالكامل\n\n"
        + f"👨‍💻 للمساعدة: {CONFIG['ADMIN_USERNAME']}"
    )


def get_security_welcome_message() -> str:
    """Returns a security-focused welcome message for returning users."""
    return (
        "مرحباً بعودتك! 🔐\n\n"
        + "✅ **حالة الأمان:** ممتازة (A+)\n"
        + "🔒 **حماية البيانات:** مشفرة بالكامل\n"
        + "🛡️ **معايير الأمان:** OWASP + NIST + GDPR\n\n"
        + "**الأوامر الأمنية:**\n"
        + "🔍 `/security_info` - معلومات الأمان التفصيلية\n"
        + "📋 `/security_audit` - تقرير التدقيق الأمني\n"
        + "🔒 `/privacy_policy` - سياسة الخصوصية\n"
        + "🛡️ `/security_headers` - رؤوس الأمان\n\n"
        + "💡 **استخدم الأزرار أدناه للوصول للميزات**\n"
        + "🎯 **نصيحة:** جرب '📊 درجات الفصل الحالي' لرؤية أحدث درجاتك"
    )


def get_support_message() -> str:
    """Returns a friendly support message with a direct contact link."""
    admin_username = CONFIG["ADMIN_USERNAME"]
    return (
        f"📞 للدعم الفني تواصل مع المطور: {admin_username}\n"
        f"أو اضغط الزر أدناه للتواصل مباشرة عبر تيليجرام."
    )


def get_help_message() -> str:
    """Returns a comprehensive help message with clear instructions and support contact."""
    admin_username = CONFIG["ADMIN_USERNAME"]
    return (
        "❓ **المساعدة والدعم**\n\n"
        "- يمكنك دائماً معرفة المزيد عن البوت أو الإبلاغ عن مشكلة من خلال الكود البرمجي على GitHub.\n"
        "- هذا الرابط متوفر للشفافية الكاملة: [GitHub](https://github.com/sispt/grade-phoenix-bot)\n\n"
        "- إذا أردت تحميل بياناتك يمكنك ذلك من الإعدادات أو التواصل مع المطور (ﻻ يشمل كلمة المرور لأننا ﻻ نحتفظ بها، فقط بيانات الدرجات).\n\n"
        "- لا يتم تخزين كلمات المرور نهائياً.\n"
        "- لا يتم فحص أو جلب درجاتك إلا عندما تطلب ذلك بنفسك.\n"
        "- لأي استفسار آخر، تواصل مع المطور."
    )


def get_error_message(error_type: str = "عام") -> str:
    """Returns enhanced error messages with helpful guidance."""
    error_messages = {
        "login_failed": "❌ تعذّر تسجيل الدخول\n\n**الحلول المقترحة:**\n• تأكد من صحة اسم المستخدم وكلمة المرور\n• تأكد من اتصالك بالإنترنت\n• جرب مرة أخرى بعد قليل\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "network_error": "🌐 حدث خطأ في الاتصال\n\n**الحلول المقترحة:**\n• تحقق من اتصالك بالإنترنت\n• جرب مرة أخرى بعد قليل\n• إذا استمرت المشكلة، تواصل مع الدعم\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "api_error": "🔧 النظام غير متاح حالياً\n\n**معلومات:**\n• قد يكون النظام في صيانة\n• جرب مرة أخرى بعد 5-10 دقائق\n• إذا استمرت المشكلة، تواصل مع الدعم\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "token_expired": "⏰ انتهت صلاحية الجلسة\n\n**الحل:**\n• اضغط '🔄 إعادة تسجيل الدخول'\n• أدخل بياناتك مرة أخرى\n• هذا طبيعي ويحدث كل فترة\n\n🔐 بياناتك آمنة ومشفرة",
        "no_grades": "📊 لا توجد درجات متاحة حالياً\n\n**الأسباب المحتملة:**\n• لم يتم نشر الدرجات بعد\n• قد تكون في فترة إجازة\n• تحقق من النظام لاحقاً\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "general": "❌ حدث خطأ غير متوقع\n\n**الحلول:**\n• جرب مرة أخرى بعد قليل\n• إذا استمرت المشكلة، تواصل مع الدعم\n• تأكد من اتصالك بالإنترنت\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
    }
    return error_messages.get(error_type, error_messages["general"])


def get_success_message(action: str) -> str:
    """Returns enhanced success messages with next steps."""
    success_messages = {
        "login": "✅ تم تسجيل الدخول بنجاح!\n\n🎉 **مرحباً بك في نظام الإشعارات الجامعية**\n\n**يمكنك الآن:**\n• 📊 فحص درجاتك الحالية\n• 📚 مراجعة درجات الفصل السابق\n• 🔔 استلام إشعارات عند تحديث الدرجات\n• ⚙️ تخصيص إعداداتك\n\n💡 **نصيحة:** جرب '📊 درجات الفصل الحالي' لرؤية أحدث درجاتك",
        "grades_updated": "📊 تم تحديث الدرجات بنجاح!\n\n✅ **تم فحص درجاتك وتحديثها**\n\n**إذا كان هناك تغييرات:**\n• ستتلقى إشعاراً فورياً\n• يمكنك مراجعة التغييرات\n• البيانات محفوظة بأمان\n\n💡 **نصيحة:** استخدم '📚 درجات الفصل السابق' لمقارنة الأداء",
        "settings_saved": "⚙️ تم حفظ الإعدادات بنجاح!\n\n✅ **تم تحديث إعداداتك الشخصية**\n\n**التغييرات سارية فوراً:**\n• إعدادات الإشعارات\n• تفضيلات العرض\n• إعدادات الخصوصية\n\n💡 **نصيحة:** يمكنك مراجعة إعداداتك في أي وقت",
        "profile_updated": "👤 تم تحديث الملف الشخصي بنجاح!\n\n✅ **تم تحديث معلوماتك الشخصية**\n\n**المعلومات المحدثة:**\n• الاسم الكامل\n• معلومات الاتصال\n• التفضيلات الشخصية\n\n💡 **نصيحة:** راجع معلوماتك بانتظام للتأكد من صحتها",
    }

    return success_messages.get(
        action,
        "✅ تم تنفيذ العملية بنجاح!\n\n🎉 **عملية ناجحة**\n\n💡 **نصيحة:** استخدم الأزرار للتنقل بين الميزات",
    )


def get_info_message(info_type: str) -> str:
    """Returns enhanced info messages with clear guidance."""
    info_messages = {
        "not_registered": "❌ لم يتم تسجيلك بعد\n\n**للبدء:**\n• اضغط على '🚀 تسجيل الدخول للجامعة'\n• أدخل اسم المستخدم وكلمة المرور\n• ستتمكن من الوصول لجميع الميزات\n\n🔐 **الأمان:**\nبياناتك مشفرة ومحمية بالكامل",
        "no_permission": "🚫 ليس لديك صلاحية الوصول\n\n**المعلومات:**\n• هذه الميزة متاحة للمطور فقط\n• إذا كنت المطور، تأكد من إعدادات الحساب\n• للمساعدة، تواصل مع الدعم\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "maintenance": "🔧 البوت في الصيانة حالياً\n\n**المعلومات:**\n• يتم إجراء تحديثات وتحسينات\n• جرب مرة أخرى بعد 10-15 دقيقة\n• إذا استمرت المشكلة، تواصل مع الدعم\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
        "coming_soon": "🚧 قريباً: هذه الميزة ستتوفر لاحقاً\n\n**الميزات القادمة:**\n• تحسينات على الواجهة\n• ميزات إضافية\n• تحسينات على الأمان\n\n💡 **نصيحة:** تابع التحديثات للحصول على أحدث الميزات",
    }

    return info_messages.get(
        info_type,
        "ℹ️ معلومات\n\n💡 **معلومات مفيدة**\n\n📞 للمساعدة: اضغط '📞 الدعم الفني'",
    )


def get_registration_success_message(username: str) -> str:
    """Returns an enhanced registration success message."""
    return (
        f"✅ تم تسجيل الدخول بنجاح!\n\n"
        f"🎉 **مرحباً {username}**\n\n"
        f"🎓 **مرحباً بك في نظام الإشعارات الجامعية**\n\n"
        f"**يمكنك الآن:**\n"
        f"• 📊 متابعة درجاتك بسهولة\n"
        f"• 🔔 استلام إشعارات فورية\n"
        f"• 📱 استخدام واجهة سهلة\n"
        f"• 🔐 الاستمتاع بحماية كاملة\n\n"
        f"💡 **:** جرب '📊 درجات الفصل الحالي' لرؤية أحدث درجاتك\n\n"
        f"🔐 **الأمان:**\n"
        f"بياناتك مشفرة ومحمية بالكامل"
    )


def get_credentials_security_info_message() -> str:
    """Returns an enhanced security info message for credential collection."""
    return (
        "🔒 **معلومات الأمان - بيانات تسجيل الدخول**\n\n"
        "**لماذا نطلب بياناتك الجامعية؟**\n"
        "نحتاج إلى اسم المستخدم وكلمة المرور فقط لتسجيل الدخول إلى نظام الجامعة وجلب درجاتك.\n\n"
        "**كيف نحمي بياناتك:**\n"
        "✅ لا نخزن كلمة المرور نهائياً\n"
        "✅ يتم حذفها فوراً بعد تسجيل الدخول\n"
        "✅ جميع البيانات مشفرة\n"
        "✅ يمكنك تغيير كلمة المرور في أي وقت\n\n"
        "**نصائح الأمان:**\n"
        "🔐 استخدم كلمة مرور قوية\n"
        "🔐 لا تشارك بياناتك مع أي شخص\n"
        "🔐 يمكنك تغيير كلمة المرور من بوابة الجامعة\n\n"
        "**للأمان الكامل:**\n"
        "ننصح باستخدام كلمة مرور خاصة لهذا الحساب وعدم مشاركتها مع أي خدمة أخرى.\n\n"
        "🔐 **بياناتك آمنة ومشفرة بالكامل**"
    )


# Database manager for message utilities
db_manager = DatabaseManager(CONFIG["DATABASE_URL"])
