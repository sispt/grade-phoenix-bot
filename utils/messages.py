...
def get_welcome_message(fullname: str | None = None) -> str:
    ...
    return (
        ...
        + f"📱 الإصدار: {CONFIG.get('BOT_VERSION', '2.5.7')}"
    )
...
def get_help_message() -> str:
    ...
    return (
        "🎓 **دليل استخدام البوت**\n\n"
        "**كيفية الاستخدام:**\n"
        "1️⃣ اضغط '🚀 تسجيل الدخول للجامعة' وأدخل بياناتك\n"
        "2️⃣ استخدم الأزرار لفحص الدرجات والإعدادات\n"
        "3️⃣ للمساعدة تواصل مع المطور أو اضغط الزر أدناه\n\n"
        "**الأوامر الأساسية:**\n"
        "📊 `/grades` - درجات الفصل الحالي\n"
        "📚 `/old_grades` - درجات الفصل السابق\n"
        "👤 `/profile` - معلوماتي الشخصية\n"
        "⚙️ `/settings` - الإعدادات والتخصيص\n"
        "📞 `/support` - الدعم الفني\n\n"
        "**أوامر الأمان:**\n"
        "🔍 `/security_info` - معلومات الأمان\n"
        "📋 `/security_audit` - تقرير التدقيق\n"
        "🔒 `/privacy_policy` - سياسة الخصوصية\n"
        "🛡️ `/security_headers` - رؤوس الأمان\n\n"
        f"👨‍💻 المطور: [{admin_username}](https://t.me/{admin_username.lstrip('@')})\n"
        f"📱 الإصدار: {CONFIG.get('BOT_VERSION', '2.5.7')}"
    )
...
def get_registration_success_message(username: str) -> str:
    ...
    return (
        ...
        f"💡 **نصيحة:** جرب '📊 درجات الفصل الحالي' لرؤية أحدث درجاتك\n\n"
        f"🔐 **الأمان:**\n"
